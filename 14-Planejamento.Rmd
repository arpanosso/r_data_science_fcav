...
## Relação entre princípios basicos e delineamentos
...
#### Vamos utilizar o operador de acesso `$` para extrair as colunas do objeto `dados`. Observe que para os fatores Tratamento e Bloco, utilizamos a função `as.factor()` que define o tipo de dados como categórico, necessário para a aplicação da análise de variância.
...
####  Utilizar a função do pacote `{ExpDes.pt}`, respeitando o delineamento experimental, no caso; Delineamento em Blocos ao Acaso (DBC)
