...
A  declaração  de  variáveis,  uma  a  uma,  é  suficiente  para  a  codificação  algorítmica  da solução  de  uma  ampla  gama  de  problemas até agora estudados. 

Entretanto, esse tipo de declaração é  insuficiente  para  resolver  um  grande número  de  problemas  computacionais.
...
Construiremos novos tipos de dados a partir da composição de tipos primitivos. 
...
Factors são utilizados para armazernar dados categorizados e são caracterizados por conterem apenas valores pré-definidos, chamados levels; e se basearem num vetor de inteiros. Dentro do objeto, os levels são organizados em ordem alfabética.
...
Matrizes e arrays são definidos usando as funções `matrix()` e `array()`, respectivamente. São objetos multidimensionais. No caso da matriz, os compartimentos, podem estar arranjados, assim:
...
No R dois argumentos são utilizados na função `matrix()` para determinar o número de colunas e forma com a qual será o preeenchimento dos elementos na matriz, por linha (`byrow=FALSE`, default) ou por coluna (`byrow=TRUE`). Compare as duas matriz do exemplo abaixo.
...
Os nomes das linhas, colunas e matrizes do array podem ser nomeados a partir de uma lista definida no argumento `dimnames` da função `array()`, como apresentado abaixo:
...
Para saber qual é o tipo de um objeto, utilizamos a função `typeof()` e `class`.
```{r}
# Array
typeof(meu_array)
class(meu_array)
...
As funções `length()`, `dim()`, `nrow()`, `ncol()` são usadas para determinar o comprimento de cada dimensão de um objeto. Observe que *atomic vector* e *factor* são estruturas unidimensionais, assim, somente a função `length()` retornará o número de elementos do objeto, e as demais função retornarão valor nulo *NULL*. 
...
Todos os objetos apresentados até agora (*atomic vector, factor, matrix* e *arrays*) podem ser classificados como estruturas de dados **HOMOGÊNEA**, pois são compostas por variáveis do mesmo tipo primitivo (inteiro, double, lógico ou caracter).
...
```
Se um conjunto homogêneo de dados é composto por variáveis do mesmo tipo, um conjunto **HETEROGÊNEO** de dados é composto por elementos que não são do mesmo tipo primitivo. No R temos as **listas** e os **data.frames**.
...
Observe que as funções `typeof()` e `class` retornam *list* e a função `length` retorna o número de *atomic vectors* utilizados na criação da lista. Funções como `dim()`, `nrow()` e `ncol()` retornarão *NULL*.
...
A expectativa é que você tenha usado uma planilha como o Excel para inserir e editar os dados e o que requer alguma prática é aprender exatamente como colocar seus números na planilha. 
...
A forma correta de inserir esses dados é ter duas colunas: uma para a variável resposta e outra para os níveis do fator experimental denominado Tratamento (controle, pré-aquecido e pré-resfriado). 
...
Vamos criar esse data.frame, primeiramente, criaremos os distintos vetores, numérico e fator.
```{r}
Resposta <- c(6.1,5.9,5.8,5.4,6.3,6.2,5.8,6.3,7.1,8.2,7.3,6.9)
Tratamento <- gl(3,4,labels = c("controle","pré-aquecido","pré-resfriado") )
```

Agrora vamos agrupar esses vetores em um data.frame.
```{r}
dados <- data.frame(Resposta, Tratamento)
dados
```

Agora, vamos extrair as informações a respeito desse data.frame:
...
As funções `str()` e `glimpse()` dos pacotes `base` e `dplyr`, respectivamente, fornecem um resumo rápido do data.frame:
```{r}
str(dados)
```
...
Lembre-se que um dataframe é um objeto bidimensional, compreendendo linhas e colunas. As linhas são referenciadas pelo primeiro subscrito (à esquerda da vírgula) , as colunas pelo segundo subscrito (à direita da vírgula). Portanto, para extrairmos a segunda observação da variável resposta `Altura_cm` usamos. Observe que o retorno é um *atomic vector* (vetor).

```{r}
dados[2,1]
```

Para extrairmos um subconjuto desse data.frame, composto somente pelos valores de altura referentes ao tratamento (método) pré-aquecido (linhas 5 a 8), vamos utilizar os `:`, deixando o índice das colunas em branco na indexação. Observe que o retorno é um data.frame.
```{r}
dados[5:8,]
```


Se o objetivo for apenas os valores, temos. 
```{r}
dados[5:8,1]
```
Observe que o retorno é um vetor.


Poderíamos extrair todos os valores a partir do operador de acesso `$`, chamando primeiramente o objeto e em seguida o nome da coluna. O retorno é um vetor.
...
Transformações nos dados podem ser realizadas utilizando esse operador, por exemplo, vamos criar uma nova variável (coluna) denominada `Altura_m` a partir da `Altura_cm`, dividindo cada valor por 100.
...
Vamos criar a coluna `Bloco` (variável) para identificação dos blocos do experimento.
...
Agora podemos modificar a posição das colunas. Observe que vamos "reciclar" o objeto `dados`, ou seja, vamos guardar esse novo data.frame no mesmo identificador. 

```{r}
dados <- dados[,c(2,4,1,3)]
dados
```
Vamor criar o vetor identificação ($ID$) para cada observação.
...
Agora vamos reordenar o data.frame por `Altura_cm` da menor para a maior (utilize a função `order()`), em seguida, da maior para a menor nota (argumento `decreasing = T`).
